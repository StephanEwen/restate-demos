...
import type { Asset, BookedOrder, EarmarkedOrder } from "../common/orders_types";
...
const DEFAULT_DELAY = 1000; // 100 msecs
...
export async function runSingleOrderProcess(script: Script, region: RegionSelector, reporter: Reporter) {
    ...
    const orderClient = regionFailoverClient<BulkOrderService>(
            { name: "order"},
            bulkOrderId,
            { regionSelector: region, timeout: 6_000 },
        reporter);
    ...
    // reverse order
    reporter.n().msg(`Cancelling order afer execution - triggering reversal of orders...`);
    await orderClient.cancel();
...
function expectOrders(actual: EarmarkedOrder[] | BookedOrder[], expected: Asset[]): void {
