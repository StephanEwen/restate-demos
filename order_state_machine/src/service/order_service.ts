...
import type { Asset, EarmarkedOrder, BookedOrder } from "../common/orders_types";
...
export const orderService = restate.object({
  ...
  handlers: {
    ...
    addOrder: async (ctx: ObjectContext<State>, asset: Asset) => {
      ...
      const order = { reservationId: ctx.rand.uuidv4(), asset };
      const success = await ctx.run("earmark", () => apis.earmark(order));
      if (!success) {
        return false;
      }
    ...
    close: async (ctx: ObjectContext<State>): Promise<boolean> => {
      ...
      const workflowId = ctx.key; // use the order key as workflow id, for simplicity
